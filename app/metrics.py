...
def get_sales_metrics():
    ...
    total_sales_value = Outflow.objects.aggregate(
        total_sales_value = Sum(F('quantity') * F('product__selling_price'))
    )['total_sales_value']
    total_sales_profit = Outflow.objects.aggregate(
        total_sales_profit = Sum(
            F('quantity') * (F('product__selling_price') - F('product__cost_price'))
        )
    )['total_sales_profit']
    
    return dict(
        total_sales=total_sales,
        total_products_sold=number_format(total_products_sold, force_grouping=True),
